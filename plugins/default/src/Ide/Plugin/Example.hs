...
exampleRules = do
  define $ \Example file -> do
    ...
    -- _pmd <- getParsedModuleDefinition file
...
codeLens ideState plId CodeLensParams{_textDocument=TextDocumentIdentifier uri} = liftIO $ do
    ...
    case uriToFilePath' uri of
      Just (toNormalizedFilePath -> filePath) -> do
        tcresult <- runIdeAction "Example.codeLens" (shakeExtras ideState) $ runMaybeT $ useE TypeCheck filePath
        ...
        case tcresult of 
          Just (tcmr, _posm) -> 
            logInfo (ideLogger ideState) (T.pack $ show tcmr)
          Nothing -> logInfo (ideLogger ideState) "Typecheck: Nothing"
...
logAndRunRequest label getResults ide pos path = do
  ...
  logInfo (ideLogger ide) $ "DIAGNOSTIC: " <> T.pack (show diag)
